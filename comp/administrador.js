var miApp = angular.module('administrador',[]);


miApp.controller('administradorctr', function($scope, $http, $rootScope, $location){
...
$scope.generar = function(){
	...
	$scope.soloEstados= function(){



		var peticion22 = {
			"url": "php/Consultas/verEstados.php",
			"method": "POST",
			"data" : {all:false
				}
		};
		...
		$scope.estad= fuebien.data;
		$scope.estados = angular.copy($scope.estad);
...
$scope.generarevenuevo = function(){
	...
	$scope.fechaelegida = function(){
		...
		modalcerrar('#mReffecha');
					...
					var peticion16 = {
						"url": "php/Cambios/agregaFechaFija.php",
						"method": "POST",
						"data": {

								 ideve: $scope.datos.fecha.id_evento,
								 fechaeve: $scope.datos.fecha.fecha,
...
$scope.lastiendas = function (){
	
	$scope.tiendavisible = true;
	$scope.productovisible = false;
	$scope.verportienda = false;
	$scope.verporcatego = false;
	$scope.eventovisible = false;
...
$scope.datost = { id:'', nombre:'', direccion: '', ciudad: '', codigo_postal:'', telefono:'', 
email:'', fax:'', latitud: '', longitud:''};
	
	$scope.modificatienda = function(){

		modalabrir('#mReftienda');
		//Creamos un objeto modal para la ventanamodal
		$scope.modi=this;


		(['id', 'nombre', 'direccion', 'ciudad', 'codigo_postal', 'telefono', 'email',
			'fax', 'latitud', 'longitud']).forEach( function(valor, indice) {

			$scope.datost[valor] = $scope.modi.tienda[valor];
...
$scope.generartienda = function(){

			switch ($scope.modalt.boton) {
				case 'Guardar':
					
					// Cerrar ventana modal
					modalcerrar('#mReftienda');
			
					var peticion23 = {
						"url": "php/Cambios/cambiarTienda.php",
						"method": "POST",
						"data": {

	
								 idti : $scope.datost.id,
								 nombreti: $scope.datost.nombre,
								 direccionti: $scope.datost.direccion,
								 ciudadti: $scope.datost.ciudad,
								 cpti: $scope.datost.codigo_postal,
								 telefonoti: $scope.datost.telefono,
								 emailti: $scope.datost.email,
								 faxti: $scope.datost.fax,
								 latitudti: $scope.datost.latitud,
								 longitudti: $scope.datost.longitud
					...
					$http(peticion23).then(function(fuebien){
				...
				case 'Añadir':
					...
					modalcerrar('#mReftienda');
						var peticion24 = {
						"url": "php/Cambios/agregarTienda.php",
						"method": "POST",
						"data": {
					...
					$http(peticion24).then(function(fuebien){
